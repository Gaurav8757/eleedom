...
function PaymentTaig({ ckycResponses, proposalResponses, token }) {
  ...
  const PaymentLinkButton = ({ paymentLink }) => {
    return (
      <CopyToClipboard text={paymentLink} disabled = {!copySuccess}>
        <button
          ...
          } group flex justify-center shadow-inner items-center text-base md:text-inherit p-1  font-medium rounded border-none active:border-none`}
        >
          <svg
            className={`${
                copySuccess
      ...
      <div className="flex flex-col max-w-sm justify-start">
        <div className="flex flex-col items-start">
          ...
          <label
            ...
            className={`flex w-10 h-10 my-3 justify-center shadow-inner text-gray-500 bg-slate-100 border border-red-500 rounded cursor-pointer 
          ...
          >
            {" "}
            <div className="my-auto text-lg font-semibold capitalize">
              {isChecked ? "Yes" : ""}{" "}
            </div>
          </label>
          ...
          <div className="flex justify-between  items-center mt-8">
            <button
              className={`transition-all  bg-green-600 text-white font-mono font-bold px-6 py-2 mr-5 rounded-lg border-green-700 border-b-[4px] hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed active:border-b-[2px] active:brightness-90 active:translate-y-[2px] active:text-black`}
              onClick={() => {
                if (paymentLink) {
